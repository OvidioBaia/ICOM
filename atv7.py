...
data = pd.read_csv("data_musica.csv")
data.columns

caracteristicas = data[['danceability', 'energy', 'loudness', 'speechiness', 'acousticness', 'instrumentalness', 'liveness', 'valence', 'tempo']]
classes = data[['nova_coluna']]
...
for i in range(10):
    ...
    clf = MLPClassifier(random_state=1,activation='tanh').fit(X_train, y_train.values.ravel())
